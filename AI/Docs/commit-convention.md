...
feat(schemas): accept public storyline dialogue context
feat(dialogue-graph): return guarded proposed events
refactor(guard): isolate hidden-field safety policy
feat(llm): add structured graph observability logs
test(guard): reject private storyline leakage
```

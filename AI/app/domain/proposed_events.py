...
from app.schemas.common import ProposedEvent
...
def _contradiction_payload(payload: DialogueRequest, reason_code: str) -> dict[str, object]:
    source_refs = payload.allowedStatement.sourceRefs
    statement_ids = _unique(payload.allowedEventPolicy.relatedStatementIds or source_refs.statementIds or [payload.allowedStatement.id])
    evidence_ids = _unique(payload.allowedEventPolicy.relatedEvidenceIds or source_refs.evidenceIds)
    timeline_ids = _unique(payload.allowedEventPolicy.relatedTimelineEventIds or source_refs.timelineIds)
...
def _contradiction_event(payload: DialogueRequest, reason_code: str) -> ProposedEvent:
    event_payload = _contradiction_payload(payload, reason_code)
...
def propose_dialogue_events(payload: DialogueRequest, *, provider_degraded: bool = False) -> list[ProposedEvent]:
    ...
    source_refs = payload.allowedStatement.sourceRefs
    related_ids = _unique(payload.allowedEventPolicy.relatedEvidenceIds or source_refs.evidenceIds)
    related_timeline_ids = _unique(payload.allowedEventPolicy.relatedTimelineEventIds or source_refs.timelineIds)
    related_statement_ids = _unique(payload.allowedEventPolicy.relatedStatementIds or source_refs.statementIds)
    ...
    if intent == "evidence" and "NOTE_CONTRADICTION_CANDIDATE_ADDED" in allowed_types:
        return [_contradiction_event(payload, intent)]
    ...
    if (
        ...
        return [
            _contradiction_event(payload, "timeline_conflict")
        ]
